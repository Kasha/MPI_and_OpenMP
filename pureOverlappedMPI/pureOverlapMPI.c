...
void update_grid_without_halo(int bx, int by, double *aold, double *anew, double *heat_ptr) ;

void update_grid_halo(int bx, int by, double *aold, double *anew, double *heat_ptr) ;
...
int main(int argc, char **argv)
{
    ...
    double start_time, end_time, mpi_time;
    ...
    for (iter = 0; iter < niters; ++iter) {
        ...
        /*Computes interior of the grid that does not depend on halo data. Non-Blocking - Overlapped since it doesn't wait for communication to arrive*/
        update_grid_without_halo(bx, by, aold, anew, &heat);

        MPI_Waitall(8, reqs, MPI_STATUSES_IGNORE);

        /*Update and compute the halo region using the newly received halo data. After Halo data was recived*/
        update_grid_halo(bx, by, aold, anew, &heat);
...
/*Compute interior of the grid that does not depend on halo data.*/
void update_grid_without_halo(int bx, int by, double *aold, double *anew, double *heat_ptr)
{
    int i, j;
    double heat = 0.0;
...
/*Update and compute the halo region using the newly received halo data.*/
void update_grid_halo(int bx, int by, double *aold, double *anew, double *heat_ptr)
{
    int j;
    double heat = (*heat_ptr);


    for ( j = 1; j <= by; j++) {

        anew[ind(1, j)] = anew[ind(1, j)] / 2.0 + (aold[ind(0, j)] + aold[ind(2, j)] +
                                    aold[ind(1, j - 1)] + aold[ind(1, j + 1)]) / 4.0 / 2.0;

        anew[ind(bx, j)] = anew[ind(bx, j)] / 2.0 + (aold[ind(bx - 1, j)] + aold[ind(bx + 1, j)] +
                                    aold[ind(bx, j - 1)] + aold[ind(bx, j + 1)]) / 4.0 / 2.0;
